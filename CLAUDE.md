...
- **Replaced by a crate.** The block checksum (`crc32c`): a CRC is the one thing that *guarantees* what
  integrity needs — every one-bit and two-bit error and every burst up to thirty-two bits detected rather than
  probable — and CRC32C is a hardware instruction on both supported targets. `rustc-hash` was already to hand
  and was refused for exactly that reason: it is built for placing keys in a table, and picking it here would
  be choosing the tool by availability. It is `pending`'s only external dependency, and it belongs there for
  the same reason serialisation belongs to whoever has a wire — the block format is that crate's.
