...
| **seal** | a block being **closed**: no more records go into it, so its bytes stop changing and a whole-block checksum becomes possible. ⚠ `seal_store_block` closes *and* writes in one call, and that merge is why the write cannot leave the worker's thread without touching everything | `RecordLog::seal_store_block` |
