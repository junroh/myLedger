...
| **flush** | a record reaching the **device**. `flush_window_hours` is how long one may go unwritten, and that is this. ⚠ the `flushed` counter does not mean this yet — it counts a survivor leaving the writeback buffer for the block being packed, which is memory; the load driver prints it as `carried on` for that reason. Design notes §20 | `PendingCapacity::flush_window_hours` |
| **seal** | a block being **closed**: no more records go into it, so its bytes stop changing and a whole-block checksum becomes possible. Closing and writing are two calls, and residency takes the block at the first of them — see **residency** | `RecordLog::seal_store_block`, `RecordLog::submit_writes` |
...
| **reply gate** | a test's hold on the pending stub's replies: nothing leaves until it is let through, and it says how many are waiting. **What replaces a latency in a test that has to see a request still waiting** — a duration is a guess about the scheduler, and a busy machine invalidates it. Permits rather than a switch, because two replies released together may be judged in either order. | `ReplyGate`, `MemoryPending::replies` |
...
| **io owner** | which writer a completion belongs to, in the top bits of its handle. Two writers on one volume draw handles from counters of their own, and a completion queue is one queue, so the poller has to be told rather than left to infer it from whether it recognises the number. | `IoOwner`, `RecordLog::take_foreign` |
