...
| **throttle** | how expiry stays behind live traffic: a bounded slice per round, nothing more offered while the last slice is unanswered, and no second walk until the index has moved. It refuses nothing and tells nobody, which is what separates it from a **rate limiter** — that word belongs to the client edge, where a refusal has someone to hear it and becomes an error to retry. Falling behind deletes late, which is the safe direction. | `expiry_per_round`, `PendingWorker::sweep_expiry`, `Sweep::waiting_at` |
