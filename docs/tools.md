...
- **Measuring what a snapshot costs needs `--snapshot-every 1`**, which dumps continuously and is what no
  ...
  to lose.
- **The two queues have two depths, because the arithmetic differs.** `--store-read-depth` is the reads'
...
- **The read queue's depth is a flag, and it bounds what a slow read can hide behind.**
  `--store-read-depth` (128) is how many reads the store holds at once; past it reads are refused and the
